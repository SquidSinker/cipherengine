...
function toalph(vect)
    join([Alphabet_CSpace.chars[i] for i in vect])
end

b = blocks(T, 130)

shifts = [18, 15, 12, 11, 4] .+ [4, 3, 3, 3, 4]

C = GridShift( - shifts, 26)

for i in b
    println(untokenise(C(i)))
end

# tokens = b.tokenised
# [toalph(i) for i in eachrow(reshape(tokens, (5, :)))]