...
function apply(S::Strange, vect::Vector{Int}; safety_checks::Txt)
...
cipher = Strange([2,2,1,5,4], 5)

a = 5
b = 5

brute_a = product([collect(1:b) for _ in 1:a]...)

optimise(x -> x(T), vec([Strange(collect(A), b) for A in brute_a]), quadgramlog)